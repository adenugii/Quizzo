...
export default function StudyGroupCard({
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      setNotification({
        type: "success",
        message: "Berhasil Masuk Grup!",
        description: "Anda akan diarahkan ke halaman detail grup...",
      });
    ...
    <>
      <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100 flex flex-col gap-3 relative">
        ...
        <div className="flex items-center gap-4 text-xs text-gray-700 mt-1">
          ...
          <span className="flex items-center gap-1"><MdOutlineBarChart className="text-base" />{quizDone}/{quizTotal} quiz</span>
        </div>
        <div>
